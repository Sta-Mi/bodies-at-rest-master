...
if __name__ ==  "__main__":
    ...
    for PARTICIPANT in participant_list:
        ...
        if opt.go200 == False:
            ...
            filename1 = FILEPATH_PREFIX+"/convnets/convnet_1_anglesDC_" + NETWORK_1 + "_200e_2e-05lr.pt"
            filename2 = None

        dev = 'cuda:0' if GPU else 'cpu'
        model = torch.load(filename1, map_location=dev, weights_only=False, encoding='latin1')
        model = model.eval()
        if GPU:
            model = model.cuda()
        print("Network 1 loaded.")
        if filename2 is not None:
            model2 = torch.load(filename2, map_location=dev, weights_only=False, encoding='latin1')
            model2 = model2.eval()
            if GPU:
                model2 = model2.cuda()
