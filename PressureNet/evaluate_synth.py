...
sys.path.insert(0, '/home/shnh/DATA/zjy')
sys.path.insert(0, '/home/shnh/DATA/zjy/smpl/smpl_webuser')
...
class PhysicalTrainer():
    ...
    def __init__(self, testing_database_file_f,
        ...
        self.opt = opt
        self.slice_start = opt.slice_start
        self.slice_end = opt.slice_end
        ...
        test_dat_f_synth = TensorPrepLib().load_files_to_database(testing_database_file_f, creation_type = 'synth', reduce_data = False, test = False)
        test_dat_m_synth = TensorPrepLib().load_files_to_database(testing_database_file_m, creation_type = 'synth', reduce_data = False, test = False)
        test_dat_f_real = TensorPrepLib().load_files_to_database(testing_database_file_f, creation_type = 'real', reduce_data = False, test = False)
        test_dat_m_real = TensorPrepLib().load_files_to_database(testing_database_file_m, creation_type = 'real', reduce_data = False, test = False)
    ...
    def init_convnet_test(self):
        ...
        s0 = self.slice_start
        s1 = self.slice_end if self.slice_end > 0 else self.test_x_tensor.size(0)
        self.test_x_tensor = self.test_x_tensor[s0:s1]
        self.test_y_tensor = self.test_y_tensor[s0:s1]
        self.slice_end_actual = s1
        ...
        if opt.go200 == False:
            self.model = torch.load(FILEPATH_PREFIX + "/convnets/convnet_1_anglesDC_" + NETWORK_1 + "_100e_2e-05lr.pt", map_location='cpu', weights_only=False, encoding='latin1')
            self.model2 = torch.load(FILEPATH_PREFIX + "/convnets/convnet_2_anglesDC_" + NETWORK_2 + "_100e_2e-05lr.pt", map_location='cpu', weights_only=False, encoding='latin1')
        else:
            self.model = torch.load(FILEPATH_PREFIX + "/convnets/convnet_1_anglesDC_" + NETWORK_1 + "_200e_2e-05lr.pt", map_location='cpu', weights_only=False, encoding='latin1')
            self.model2 = None
    ...
    def val_convnet_special(self, epoch):
        if GENDER == "f":
            model_path = '/home/shnh/DATA/zjy/smpl/models/basicModel_f_lbs_10_207_0_v1.0.0.pkl'
        else:
            model_path = '/home/shnh/DATA/zjy/smpl/models/basicmodel_m_lbs_10_207_0_v1.0.0.pkl'
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for batch_idx, batch in enumerate(self.test_loader):
                ...
                if viz_type == "arm_penetration":
                    ...
                    ims_to_display.append(INPUT_DICT['batch_mdm'].detach().cpu().numpy().reshape(64, 27)*-1) #Q-, GT
                    ims_to_display.append(INPUT_DICT['batch_images'][0, 4, :]*23.) #pmat
                    ims_to_display.append(INPUT_DICT['batch_images'][0, 2, :]*45.) #Q-, 1
                    ims_to_display.append(OUTPUT_DICT['batch_mdm_est'].detach().cpu().numpy().reshape(64, 27)*-1) #Q-, 2
                    ims_to_display.append(INPUT_DICT['batch_mdm'].detach().cpu().numpy().reshape(64, 27)*-1) #Q-, GT
                    segment_limbs = True
    ...
    def val_convnet_general(self, epoch):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for batch_idx, batch in enumerate(self.test_loader):
                ...
                q = OUTPUT_DICT['batch_mdm_est'].detach().cpu().numpy().reshape(OUTPUT_DICT['batch_mdm_est'].size()[0], 64, 27) * -1
                q = np.mean(q, axis=0)
        ...
        suffix = ''
        if self.slice_end > 0:
            suffix = '_slice%d_%d' % (self.slice_start, self.slice_end_actual)
        pkl.dump(RESULTS_DICT, open(dir+'/results_synth_'+TESTING_FILENAME+suffix+'.p', 'wb'))
...
if __name__ == "__main__":
    ...
    p.add_option('--file_idx', action='store', type='int', dest='file_idx', default=None,
                 help='Only run one file (0-based index) from the selected testing block.')

    p.add_option('--slice_start', action='store', type='int', dest='slice_start', default=0,
                 help='Start sample index for slicing the loaded test file.')

    p.add_option('--slice_end', action='store', type='int', dest='slice_end', default=-1,
                 help='End sample index for slicing the loaded test file (-1 = all).')
    ...
    if opt.hd == False:
        FILEPATH_PREFIX = "/home/zjy/bodies-at-rest-master/data_BR"
    else:
    ...
    if opt.file_idx is not None:
        testing_filename_list = [testing_filename_list[opt.file_idx]]
